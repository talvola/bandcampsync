...
class PlexCollection:
    ...
    def find_album(self, artist, title, local_path=None):
        """Resolve one album to a ratingKey, or None if Plex has not scanned it yet.

        Searched by title within the music section and confirmed against the artist,
        because Plex indexes what the *tags* say. That is the point of asking Plex at all:
        it sees an album whatever its directory is called and wherever it sits in the
        tree, which neither the id files nor the directory names manage on their own.
        """
        ...
        # Prefer an artist+title agreement; fall back to a unique title match, which is
        # normal when the blog credits an artist differently from the release tags.
        ...
        if local_path:
            wanted = _norm(Path(local_path).name)
            for entry in candidates:
                if _norm(f"{entry.get('parentTitle', '')} - {entry.get('title', '')}") == wanted:
                    return entry["ratingKey"]
        exact = [e for e in candidates if _norm(e.get("title", "")) == want_title]
        if len(exact) == 1:
            return exact[0]["ratingKey"]
        return None
...
def drain_pending(state, collection, apply=False):
    """Add everything queued that Plex has now scanned. Returns (added, still_pending).

    Items Plex has not seen yet are left in the queue untouched - that is the whole
    mechanism for not having to wait on a scan.
    """
